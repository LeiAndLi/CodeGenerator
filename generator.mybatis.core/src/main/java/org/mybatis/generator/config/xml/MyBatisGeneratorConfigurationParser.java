...
import org.mybatis.generator.config.IgnoreColumList;
...
import org.mybatis.generator.config.PlanTable;
import org.mybatis.generator.config.PluginConfiguration;
import org.mybatis.generator.config.PropertyHolder;
import org.mybatis.generator.config.SecurityColumn;
import org.mybatis.generator.config.SqlMapExtGeneratorConfiguration;
...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) throws XMLParserException, IOException {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if("sqlMapExtGenerator".equals(childNode.getNodeName())){
                parseSqlMapExtGenerator(context, childNode);
            } else if ("javaClientGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaClientGenerator(context, childNode);
            } else if ("javaClientExtGenerator".equals(childNode.getNodeName())){
                parseJavaClientExtGenerator(context, childNode);
            } else if ("table".equals(childNode.getNodeName())) { //$NON-NLS-1$
                TableConfiguration tc = parseTable(context, childNode);
                tc.setPlan(false);
                if(tc.getPlanTable() != null){
                    TableConfiguration ptc = parseTable(context, childNode);
                    ptc.setTableName(tc.getPlanTable().getTableName());
                    ptc.setDomainObjectName(tc.getPlanTable().getDomainObjectName());
                    ptc.setPlan(true);
                    tc.setPtc(ptc);
                }
                
            } else if ("javaVoGenerator".equals(childNode.getNodeName())){
            ...
            } else if ("javaServiceImplementGenerator".equals(childNode.getNodeName())){
            	parseJavaServiceImplGenerator(context, childNode);
            } else if ("javaRpcGenerator".equals(childNode.getNodeName())){
            	parseJavaRpcGenerator(context, childNode);
            } else if ("javaFacadeGenerator".equals(childNode.getNodeName())){
            	parseJavaFacadeGenerator(context, childNode);
            }else if ("javaFacadeImplementGenerator".equals(childNode.getNodeName())){
            	parseJavaFacadeImplGenerator(context, childNode);
            }else if ("javaRPCHttpApiTestGenerator".equals(childNode.getNodeName())){
            	parseRpcHttpApiTestGenerator(context, childNode);
            }else if("javaDTOParamGenerator".equals(childNode.getNodeName())){
                parseJavaDTOParamGenerator(context, childNode);
            }else if("javaDTOAdapterGenerator".equals(childNode.getNodeName())){
                parseJavaDTOAdapterGenerator(context, childNode);
            }else if("javaDTOAdapterImplementGenerator".equals(childNode.getNodeName())){
                paserJavaDTOAdapterImplementGenerator(context, childNode);
            }else if("javaVOAdapterGenerator".equals(childNode.getNodeName())){
                parseJavaVOAdapterGenerator(context, childNode);
            }else if("javaVOAdapterImplementGenerator".equals(childNode.getNodeName())){
                parseJavaVOAdapterImplementGenerator(context, childNode);
            }else if ("templateUserdefGenerator".equals(childNode.getNodeName())){
            	parseUserdefGenerator(context, childNode);
            }else if ("ignoreColumns".equals(childNode.getNodeName())){
            	parseIgnoreColumns(context, childNode);
            }
           
        }
    }
    
    protected void parseIgnoreColumns(Context context, Node node){
    	IgnoreColumList iglist = new IgnoreColumList();
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("ignoreColumn".equals(childNode.getNodeName())) { //$NON-NLS-1$
            	parseIgnoreColumnInModel(iglist, childNode);
            }
        }
        
        context.setIgnoreColumnList(iglist);
    }
    
    protected void parseJavaVoGenerator(Context context, Node node){
    	javaVoGeneratorConfig config = new javaVoGeneratorConfig();
    	context.setVoGeneratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("columnAdd".equals(childNode.getNodeName())) { //$NON-NLS-1$
            	ColumnAdd col = parseColumnAdd(childNode);
            	config.addAddColumn(col);
            }
        }
    }
    
    protected ColumnAdd parseColumnAdd(Node node){
    	Properties attributes = parseAttributes(node);
    	
    	String dbSubFix = attributes.getProperty("dbSubFix"); //$NON-NLS-1$
    	String addSubFix = attributes.getProperty("addSubFix"); //$NON-NLS-1$
    	String type = attributes.getProperty("type"); //$NON-NLS-1$
    	
    	ColumnAdd col = new ColumnAdd(dbSubFix, addSubFix, type);
    	return col;
    }
    
    protected void parseJavaServiceGenerator(Context context, Node node){
    	javaServiceGeneratorConfig config = new javaServiceGeneratorConfig();
    	context.setServiceGeneratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaServiceImplGenerator(Context context, Node node){
    	javaServiceImplementGeneratorConfig config = new javaServiceImplementGeneratorConfig();
    	context.setServImplGeneratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaRpcGenerator(Context context, Node node){
    	javaRpcGeneratorConfig config = new javaRpcGeneratorConfig();
    	context.setRpcGenratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaFacadeGenerator(Context context, Node node){
    	javaFacadeGeneratorConfig config = new javaFacadeGeneratorConfig();
    	context.setFacadeGeneratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaFacadeImplGenerator(Context context, Node node){
    	JavaFacadeImplGeneratorConfig config = new JavaFacadeImplGeneratorConfig();
    	context.setFacadeImplGeneratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseRpcHttpApiTestGenerator(Context context, Node node){
    	JavaRpcTestGeneratorConfig config = new JavaRpcTestGeneratorConfig();
    	context.setRpcTestGeneratorConfig(config);
    	
    	this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaDTOParamGenerator(Context context, Node node){
        JavaDTOParamGeneratorConfig config = new JavaDTOParamGeneratorConfig();
        context.setDtoParamGeneratorConfig(config);
        
        this.parseOtherJavaGenerator(context, node, config); 
    }
    
    protected void parseJavaDTOAdapterGenerator(Context context, Node node){
        JavaDTOAdapterGeneratorConfig config = new JavaDTOAdapterGeneratorConfig();
        context.setDtoAdapterGeneratorConfig(config);
        this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void paserJavaDTOAdapterImplementGenerator(Context context, Node node){
        JavaDTOAdapterImplementGeneratorConfig config = new JavaDTOAdapterImplementGeneratorConfig();
        context.setDtoAdapterImplementGeneratorConfig(config);
        this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaVOAdapterGenerator(Context context, Node node){
        JavaVOAdapterGeneratorConfig config = new JavaVOAdapterGeneratorConfig();
        context.setVoAdapterGeneratorConfig(config);
        this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseJavaVOAdapterImplementGenerator(Context context, Node node){
        JavaVOAdapterImplementGeneratorConfig config = new JavaVOAdapterImplementGeneratorConfig();
        context.setVoAdapterImplementGeneratorConfig(config);
        this.parseOtherJavaGenerator(context, node, config);
    }
    
    protected void parseUserdefGenerator(Context context, Node node){
    	UserdefGeneratorConfig config = new UserdefGeneratorConfig();
    	context.addUserdefGeneratorConfigs(config);
    	
    	Properties attributes = parseAttributes(node);
    	
    	String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String templateFileName = attributes.getProperty("templateFileName"); //$NON-NLS-1$
        String targetFileName = attributes.getProperty("targetFileName");
        String enableAppend = attributes.getProperty("enableAppend");
        
        config.setTargetPackage(targetPackage);
        config.setTargetProject(targetProject);
        config.setTemplateFileName(templateFileName);
        config.setTargetFileName(targetFileName);
        
        	
        if (stringHasValue(enableAppend)) {
        	config.setEnableAppend(isTrue(enableAppend));
        }
        
        /**
         * if (stringHasValue(enableVo)) {
            config.setEnableVo(isTrue(enableVo));
        }
         */
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(config, childNode);
            }
        }
    }
    ...
    protected void parseSqlMapExtGenerator(Context context, Node node) {
        SqlMapExtGeneratorConfiguration sqlMapGeneratorConfiguration = new SqlMapExtGeneratorConfiguration();

        context.setSqlMapExtGeneratorConfiguration(sqlMapGeneratorConfiguration);
    ...
    protected TableConfiguration parseTable(Context context, Node node) {
        ...
        String enableBatchDelete = attributes.getProperty("enableBatchDelete");
        String enableBatchUpdate = attributes.getProperty("enableBatchUpdate");
        String enableSelectByCondition = attributes.getProperty("enableSelectByCondition");
        ...
        if (stringHasValue(enableBatchInsert)) {
        	tc.setEnableBatchInsert(
        			isTrue(enableBatchInsert));
        }
        
        if (stringHasValue(enableBatchDelete)) {
        	tc.setEnableBatchDelete(
        			isTrue(enableBatchDelete));
        }
        
        if (stringHasValue(enableBatchUpdate)){
        	tc.setEnableBatchUpdate( 
        			isTrue(enableBatchUpdate));
        }
        
        if (stringHasValue(enableSelectByCondition)){
        	tc.setEnableSelectByCondition(isTrue(enableSelectByCondition));
        }
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if("securityColumn".equals(childNode.getNodeName())){
            	parseSecurityColumn(tc, childNode);
            } else if("planTable".equals(childNode.getNodeName())){
                parsePlanTable(tc, childNode);
            }
        }
    ...
    private void parseSecurityColumn(TableConfiguration tc, Node node){
    	Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$
        String javaType = attributes.getProperty("java_type"); //$NON-NLS-1$
        String jdbcType = attributes.getProperty("jdbc_type"); //$NON-NLS-1$
        
        SecurityColumn sc = new SecurityColumn(column, javaType, jdbcType);
        tc.addSecurityColumns(sc);
    }
    
    private void parsePlanTable(TableConfiguration tc, Node node){
        Properties attributes = parseAttributes(node);
        String tableName = attributes.getProperty("tableName"); //$NON-NLS-1$
        String domainObjectName = attributes.getProperty("domainObjectName"); //$NON-NLS-1$
        
        PlanTable config = new PlanTable(tableName, domainObjectName);
        tc.setPlanTable(config);
    }
    
    private void parseIgnoreColumnInModel(IgnoreColumList iglist, Node node){
    	Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$
        String delimitedColumnName = attributes
                .getProperty("delimitedColumnName"); //$NON-NLS-1$
    ...
    protected void parseJavaModelGenerator(Context context, Node node) {
        ...
        javaModelGeneratorConfiguration.setGenerate(parseGenerate(attributes));
    ...
    protected boolean parseGenerate(Properties attributes){
    	String generate = attributes.getProperty("generate"); //$NON-NLS-1$
    ...
    private void parseJavaClientGenerator(Context context, Node node) {
        ...
        javaClientGeneratorConfiguration.setGenerate(parseGenerate(attributes));
    ...
    private void parseJavaClientExtGenerator(Context context, Node node) {
        JavaClientExtGeneratorConfiguration javaClientGeneratorConfiguration = new JavaClientExtGeneratorConfiguration();

        context.setJavaClientExtGeneratorConfiguration(javaClientGeneratorConfiguration);
    ...
    private void parseOtherJavaGenerator(Context context, Node node, otherjavaGeneratorConfiguration config){
    	Properties attributes = parseAttributes(node);
    	
    	String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String nameSubfix = attributes.getProperty("nameSubfix"); //$NON-NLS-1$
        
        config.setTargetPackage(targetPackage);
        config.setTargetProject(targetProject);
        config.setNameSubfix(nameSubfix);
        config.setGenerate(parseGenerate(attributes));
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("methodParam".equals(childNode.getNodeName())){
            	parseMethodParam(config, childNode);
            }
            
            if ("import".equals(childNode.getNodeName())){
            	parseImport(config, childNode);
            }
            
            if ("annotation".equals(childNode.getNodeName())){
            	parseAnnotation(config, childNode);
            }
        }
    }
    
    
    private void parseMethodParam(otherjavaGeneratorConfiguration config, Node node){
    	Properties attributes = parseAttributes(node);
    	
    	MethodParamConfig cfg = new MethodParamConfig();
    	
    	String name = attributes.getProperty("name"); //$NON-NLS-1$
    	String type = attributes.getProperty("type"); //$NON-NLS-1$
    	String annotation = attributes.getProperty("annotation"); //$NON-NLS-1$
    	
    	cfg.setName(name);
    	cfg.setType(type);
    	cfg.setAnnotation(annotation);
    	
    	config.addMethodConfig(cfg);
    }
   
    private void parseImport(otherjavaGeneratorConfiguration config, Node node){
    	Properties attributes = parseAttributes(node);
    	...
    	config.addImportConfig(cfg);
    }
    
    private void parseAnnotation(otherjavaGeneratorConfiguration config, Node node){
    	Properties attributes = parseAttributes(node);
