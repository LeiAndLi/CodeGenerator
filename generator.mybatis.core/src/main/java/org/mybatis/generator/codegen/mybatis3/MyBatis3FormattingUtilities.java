...
public class MyBatis3FormattingUtilities {
    ...
    public static String getParameterClause(
    	...
    	if(introspectedColumn.isSecurity()){
    		return getEncryptColumnName(introspectedColumn, prefix);
    	}
    ...
    public static String getEncryptColumnName(IntrospectedColumn introspectedColumn, String prefix){
    	
    	
    	StringBuilder sb = new StringBuilder();
    	String org = escapeStringForMyBatis3(introspectedColumn.getJavaProperty(prefix));
    	
    	sb.append("aes_encrypt(#{"+org+", jdbcType="+introspectedColumn.getSecurityJdbcType()+"},#{");
    	if(prefix != null){
    		sb.append(prefix);
    	}
    	sb.append("secretKey,jdbcType=VARCHAR})");
    	return sb.toString();
    }
    
    public static String getDecrityColumnName(IntrospectedColumn introspectedColumn){
    	StringBuilder sb = new StringBuilder();
    	String org = escapeStringForMyBatis3(introspectedColumn
                .getActualColumnName());
    	
    	sb.append("cast( aes_decrypt("+org+",#{secretKey,jdbcType=VARCHAR})  as char ) as " + org);
    	return sb.toString();
    }
