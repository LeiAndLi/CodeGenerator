...
public class MyBatisGenerator {
    ...
    public void generate(ProgressCallback callback) throws SQLException,
            IOException, InterruptedException, XMLParserException {
    ...
    public void generate(ProgressCallback callback, Set<String> contextIds)
            throws SQLException, IOException, InterruptedException, XMLParserException {
        ...
        try {
            ...
            if (targetFile.exists()) {
            	if(shellCallback.isOverwriteEnabled()){
                    ...
                    warnings.add(getString("Warning.11", //$NON-NLS-1$
                            targetFile.getAbsolutePath()));
                    System.out.println("linan-overwite source");
            	}else if(gxf.isMergeable()){
            		source = XmlFileMergerJaxp.getMergedSource(gxf,
                            targetFile);
                    System.out.println("linan-merge source");
            	}else {
                    source = gxf.getFormattedContent();
                    ...
                    warnings.add(getString(
                            "Warning.2", targetFile.getAbsolutePath())); //$NON-NLS-1$
                    System.out.println("linan-3");
                }
            } else {
                source = gxf.getFormattedContent();
                System.out.println("linan-4");
            }
